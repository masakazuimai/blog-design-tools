/*
 * テンプレート4：フラワー
 * カードを中心から放射状（花びら状）に並べ、円盤ごと回す。tiltで見下ろし角を変え、
 * 真上に近づけるほど「上から見た花が回る」見え方になる。構造・CSSは共通（scene>tilt>stage>.c3-card）。
 * tilt=rotateX（見下ろし角）／stage=rotateZ（スピン）／card=rotateZ(i*step) translateY(-radius)（花びら配置）。
 */
const SENS = 0.22;
...
const fields = [
  { key: "count", min: 4, max: 16, step: 1, def: 8 },
  { key: "radius", min: 80, max: 420, step: 10, def: 200 },
  { key: "perspective", min: 600, max: 2200, step: 50, def: 1100 },
  { key: "cardW", min: 80, max: 240, step: 10, def: 130 },
  { key: "cardH", min: 120, max: 320, step: 10, def: 200 },
  { key: "tilt", min: 20, max: 85, step: 1, def: 60, label: { ja: "見下ろし", en: "look down" } },
  { key: "speed", min: 4, max: 40, step: 1, def: 14 },
  { key: "hue", min: 0, max: 360, step: 5, def: 330, hue: true },
];
...
function mount(host, state, autoOn) {
  ...
  function tick(time, deltaTime) {
    ...
    for (let i = 0; i < cards.length; i++) {
      // 手前（下）に来た花びらを明るく、奥（上）を暗く
      const a = ((s.rotation + i * step - 90) * Math.PI) / 180;
...
function scriptBody(p, speed) {
  ...
  gsap.ticker.add(function (time, dt) {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      cards[i].style.filter = "brightness(" + (0.5 + 0.5 * (f * 0.5 + 0.5)).toFixed(3) + ")";
    }
  });
...
function fullCode(p, autoOn, isEn) {
  ...
  const note = isEn
    ? "3D flower carousel | GSAP | replace each .c3-card content with your own <img>"
    : "3Dフラワーカルーセル | GSAP製 | 各 .c3-card の中身は自由に <img> 等へ差し替え";
  return `<!-- ${note} -->
<div class="c3-scene">
  <div class="c3-tilt">
    <div class="c3-stage">
