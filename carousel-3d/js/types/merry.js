/*
 * テンプレート1：メリーゴーランド（横回転リング）
 * カードを円環状に並べ、GSAPのticker＋Draggable＋InertiaPluginで自動回転＋ドラッグ慣性回転する。
 */
const SENS = 0.22;
...
const fields = [
  { key: "count", min: 3, max: 12, step: 1, def: 8 },
  { key: "radius", min: 180, max: 640, step: 10, def: 340 },
  { key: "perspective", min: 600, max: 2400, step: 50, def: 1200 },
  { key: "cardW", min: 120, max: 320, step: 10, def: 200 },
  { key: "cardH", min: 120, max: 360, step: 10, def: 260 },
  { key: "tiltX", min: -45, max: 45, step: 1, def: 6, label: "tilt X" },
  { key: "tiltZ", min: -45, max: 45, step: 1, def: 0, label: "tilt Z" },
  { key: "speed", min: 4, max: 40, step: 1, def: 16 },
  { key: "hue", min: 0, max: 360, step: 5, def: 210, hue: true },
];
...
function mount(host, state, autoOn) {
  ...
  function layout() {
    ...
    tilt.style.transform = `rotateX(${s.tiltX}deg) rotateZ(${s.tiltZ}deg)`;
    cards.forEach((card, i) => {
      ...
      card.style.transform = `rotateY(${i * step}deg) translateZ(${s.radius}px)`;
    });
  }
...
function styleBlock(p) {
  const h = Math.round(p.cardH + 180);
  return `<style>
...
function scriptBody(p, speed) {
  ...
  var S = { count: ${p.count}, radius: ${p.radius}, cardW: ${p.cardW}, cardH: ${p.cardH}, tiltX: ${p.tiltX}, tiltZ: ${p.tiltZ}, speed: ${speed}, perspective: ${p.perspective} };
...
function fullCode(p, autoOn, isEn) {
  ...
  const note = isEn
    ? "3D carousel (merry-go-round) | GSAP | replace each .c3-card content with your own <img>"
    : "3Dカルーセル（メリーゴーランド）| GSAP製 | 各 .c3-card の中身は自由に <img> 等へ差し替え";
  return `<!-- ${note} -->
<div class="c3-scene">
  <div class="c3-tilt">
    <div class="c3-stage">
...
export const merryType = {
  id: "merry",
  label: { ja: "メリーゴーランド", en: "Carousel" },
  hasAuto: true,
  ...
  fullCode,
  meta(p, autoOn) {
    return `${p.count}枚 / r${p.radius} / 前後${p.tiltX}・横${p.tiltZ} / ${autoOn ? p.speed + "s" : "停止"}`;
  },
};
