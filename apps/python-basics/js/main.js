...
const STORE_KEY = 'cqPythonBasicsQuizV1';
...
// コード実行（SkulptでPythonを実行し、print出力を捕捉して表示）
$('runBtn').onclick = () => {
  ...
  Sk.configure({
    ...
    read: f => {
      if (Sk.builtinFiles === undefined || Sk.builtinFiles['files'][f] === undefined) {
        throw new Error('モジュールが見つかりません: ' + f);
      }
  ...
  Sk.misceval.asyncToPromise(() => Sk.importMainWithBody('<stdin>', false, code, true))
    .then(() => {
      out.textContent = lines.length ? lines.join('') : '(出力なし — print() で表示できます)';
    })
    .catch(err => {
      const msg = String(err);
      // 途中までの出力は通常色で残し、エラー行だけを赤くする
      if (lines.length) {
        out.className = '';
